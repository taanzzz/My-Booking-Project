    ...
    <div className="p-4 max-w-4xl mx-auto space-y-10">
      ...
      {showModal && (
        <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black/30 z-50">
          <div className="bg-gradient-to-br from-white to-gray-100 dark:from-gray-800 dark:to-gray-700 p-6 rounded-xl shadow-xl w-96 relative">
            <h2 className="text-xl font-bold mb-2 text-gray-800 dark:text-white">Confirm Booking</h2>
            <p className="text-gray-700 dark:text-gray-200">{room.name}</p>
            <p className="text-sm text-gray-600 dark:text-gray-300">Price: ${room.price}</p>
            <DatePicker
      ...
      <div className="p-6 rounded-xl shadow-xl bg-gradient-to-br from-yellow-50 to-orange-100 dark:from-yellow-900 dark:to-orange-900">
        ...
        {reviews.length > 0 ? (
            ...
            {reviews.map((review) => (
              ...
              >
                <p className="text-base font-semibold text-gray-800 dark:text-white">
                  {review.username} <span className="text-xs text-gray-500 dark:text-gray-300">({review.userEmail})</span>
                </p>
                <p className="text-yellow-600 dark:text-yellow-400 text-sm">Rating: {review.rating}⭐</p>
                <p className="text-sm mt-1 text-gray-700 dark:text-gray-200">{review.comment}</p>
          ...
          <p className="text-gray-600 dark:text-gray-300">No reviews available for this room yet.</p>
        )}
      </div>
    </div>
  </div>
